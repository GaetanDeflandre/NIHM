...
public class GestureClass {
	...
	public void computeKmeansLearner() {
		...
		// kmeanslearner = new KMeansLearner(2, new OpdfIntegerFactory(19),
		// obsVectors);
		// hmm = kmeanslearner.learn();
		hmm = trainHMM(obsVectors);
	}

	public Hmm<ObservationInteger> trainHMM(
			ArrayList<ArrayList<ObservationInteger>> obsVectors) {
		...
		do {
			...
			KMeansLearner<ObservationInteger> kml = new KMeansLearner<ObservationInteger>(
					numberOfHiddenStates, new OpdfIntegerFactory(19),
					obsVectors);
		...
		System.out.println("Number of hidden states for " + gestureClassName
				+ " : " + (numberOfHiddenStates - 1));

		return trainedHmm;
	}

	public void printObservationsSequences(
			ArrayList<ArrayList<ObservationReal>> obsVectors) {
	...
	public void printObservationsSequences2(
			ArrayList<ArrayList<ObservationInteger>> obsVectors) {
	...
	public double computeScore(ArrayList<Double> featuresRawPoints) {
		...
		ForwardBackwardCalculator fbc = new ForwardBackwardCalculator(obs,
				this.hmm);
