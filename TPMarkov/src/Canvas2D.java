	...
	public void paint(Graphics g) {
		...
		if (!TStroke.isEmpty()) {
			...
			for (int i = 1; i < TStroke.size(); i++) {
				g.drawLine(TStroke.get(i - 1).getPoint().x, TStroke.get(i - 1)
						.getPoint().y, TStroke.get(i).getPoint().x, TStroke
						.get(i).getPoint().y);
				g.drawArc(TStroke.get(i - 1).getPoint().x - r,
						TStroke.get(i - 1).getPoint().y - r, 2 * r, 2 * r, 0,
						360);
			}
			g.drawArc(TStroke.get(TStroke.size() - 1).getPoint().x - r, TStroke
					.get(TStroke.size() - 1).getPoint().y - r, 2 * r, 2 * r, 0,
					360);
		}
		...
		if (!TresampledPoints.isEmpty()) {
			...
			for (int i = 0; i < TresampledPoints.size(); i++) {
				g.drawArc(TresampledPoints.get(i).x - r,
						TresampledPoints.get(i).y - r, 2 * r, 2 * r, 0, 360);
			}
		}
