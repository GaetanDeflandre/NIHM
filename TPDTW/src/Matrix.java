...
class Matrix {
	...
	public Matrix invert(double[] rdet) {
		...
		/*
		 * Allocate permutation vectors for l and m, with the same origin as the
		 * matrix.
		 */
		...
		for (k = 0; k < nRows; k++) {
			...
			/*
			 * Divide column by minus pivot (value of pivot element is contained
			 * in biga).
			 */
			if (biga == 0.0) {
				rdet[0] = 0.;
	...
	/*
	 * public Matrix slice(BitVector rowmask, BitVector colmask){ Matrix r = new
	 * Matrix(rowmask.bitCount (nRows), colmask.bitCount (nCols)); int ri = 0;
	 * for (int i = 0; i < nRows; i++) if (rowmask.isSet (i) ) { int rj = 0; for
	 * (int j = 0; j < nCols; j++) if(colmask.isSet (j)) r.items[ri][rj++] =
	 * items[i][j]; ri++; } return r; }
	 * 
	 * public Matrix deSlice (double fill, int nrow, int ncol, BitVector
	 * rowmask, BitVector colmask){ // *** not optimal : result is copied back
	 * Matrix r = new Matrix(nrow, ncol); r.fill (fill); int ri = 0; for (int i
	 * = 0; i < nrow; i++) { if (rowmask.isSet (i) ) { int rj = 0; for (int j =
	 * 0; j < ncol; j++) if (colmask.isSet (j)) r.items[i][j] = items[ri][rj++];
	 * ri++; } } return r; }
	 */
